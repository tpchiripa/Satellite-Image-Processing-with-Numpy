...
| 1 | Spectral index engine: NDVI, NBR, dNBR (deterministic, unit-tested) | next |
| 2 | Wildfire detection on sample imagery: burned-area mask, severity | planned |
...
src/
    ...
    monitoring/          event store, time-series tracking
...
- **NASA FIRMS** — near-real-time active-fire detections (free `MAP_KEY`, 5,000 requests / 10-min window)
...
Run the test suite to confirm the project skeleton and evidence-level
types are working:
...
Optional: bring up the PostgreSQL + PostGIS database (not yet used by
any code — wired in ahead of Milestone 3):
